...
u_sd_Group_true=c(u2s,u3s)
data <- data %>%
  # 添加关于被试的随机截距和随机斜率
  add_ranef(c("subj","type"), u0s, u1s) %>%
  # 添加关于被试的组别的随机截距和随机斜率
  add_ranef(c("Group","type"), u2s, u3s) %>%
  # 添加观察值的误差项
  add_ranef(sigma = 0.1)
```
...
```{r}
data <-data %>%
  mutate(LPP = b0 +(b1+u0s+u2s)* type + b2 * Group + b3*type*Group+u0s+u3s+ sigma)
head(data,10) #查看生成的数据矩阵
```
# 数据分析
...
```{r}
model_1 <- brm(LPP ~ Group + type, data = data,family="gaussian",cores = 4,chains = 4,warmup =500,iter = 1000)
summary(model_1)
plot(model_1)
...
```{r}
model_3 <- brm(LPP ~ Group * type +(1|subj)+(1+Group), data = data,family="gaussian",cores = 4,chains = 4,warmup =500,iter = 1000)
summary(model_3)
plot(model_3)
```
在前面的代码基础之上加上随机斜率。探究在不同被试，不同组别下图片对LPP波幅的影响。
```{r}
model_4 <- brm(LPP ~ Group * type +(1+type|Group)+(1+type|subj), data = data,family="gaussian",cores = 4,chains = 4,warmup =500,iter = 1000)
summary(model_4)
plot(model_4)
```
